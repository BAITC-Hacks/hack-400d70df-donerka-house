...
Из карточки EKT также читается актуальный статус наличия текущего региона: `В наличии` или `Под заказ`. Для доступного товара ассистент передаёт региональный лимит количества (`stock_quantity`) и город (`stock_location`), если сайт их публикует. Поэтому он может ответить, например: «В наличии в Астане, до 8 шт.» или «Под заказ». Полная разбивка остатков по всем складам не является публичной частью страницы; для неё нужен официальный доступ EKT к закрытому API. OpenAI API-ключ не является ключом EKT и не используется для обхода этой авторизации.
