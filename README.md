...
При старте сервер загружает локальный data/detail.json и пытается получить остальные detail-записи по url_api_detail из двух JSON-файлов. Сетевую загрузку можно отключить переменной LOAD_REMOTE_DETAILS=false. Если detail API недоступен, ассистент не выдумывает наличие и сообщает об ограничении. Сертификаты, которых нет в исходных данных, помечаются как демо-ссылки и требуют подтверждения у поставщика.
