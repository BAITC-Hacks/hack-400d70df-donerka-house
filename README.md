...
| `GET` | `/api/cart` | Получить корзину аккаунта или текущей браузерной сессии |
...
Гостевая корзина привязана к HttpOnly-cookie `ekt_session`, а корзина аккаунта — к серверной сессии пользователя. Для production следует заменить in-memory корзину на Redis или базу данных, чтобы корзины работали между несколькими экземплярами.
