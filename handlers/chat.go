...
5. При намерении купить товар сначала уточни/зафиксируй товар и количество, затем попроси явное подтверждение «да, добавь». Корзина меняется только после такого подтверждения.

Важно:
- Никогда не считай фразы «хочу купить», «беру», «дай 5 шт» или «добавь 5 шт» финальным подтверждением. Сначала покажи товар и количество и попроси «да, добавь».
- Только отдельное подтверждение клиента «да, добавь», «подтверждаю» или эквивалент после подготовленного запроса может менять корзину.
- Если товар недоступен, предложи релевантные аналоги и кратко объясни сходство.
- Если в данных есть ссылки на сертификаты, обязательно сообщи их. Если сертификатов нет в данных, не выдумывай их.
- По условиям покупки: доставка по Казахстану; онлайн-оплата AirbaPay/Cloudpayments и рассрочка. Для прототипа минимальная партия — 1 шт., если для конкретного товара не указано иное.
...
var quantityFollowupPurchasePattern = regexp.MustCompile(`(?i)^\s*(?:(?:дай|дайте)(?:\s+мне)?|добав(?:ь|ьте)(?:\s+мне)?|беру|возьму)?\s*\d+\s*(?:шт\.?|штук(?:и)?|pcs)\s*package handlers
...
var confirmationPattern = regexp.MustCompile(`(?i)^\s*(?:да(?:\s*,?\s*добав(?:ь|ьте))?|подтверждаю|согласен|согласна|yes|confirm)\s*[.!]?\s*package handlers
...
var cancellationPattern = regexp.MustCompile(`(?i)^\s*(?:нет|не\s+надо|отмена|отмени|cancel)\s*[.!]?\s*package handlers
...
func productUnavailable(product models.Product) bool {
	return strings.EqualFold(product.Availability, "Под заказ") ||
		(product.Availability != "" && product.StockQuantity == 0 && product.TotalStockQuantity == 0)
}
